...
const Users = () => {
  ...
  useEffect(() => {
    axios('https://viridian-confusion-henley.glitch.me/user').then(res =>
      setUser(res.data)
    );
  }, []);
  ...
  return (
    <Flex direction={'column'} bg="white" borderRadius={'10px'}>
      <Flex
        justify={'flex-end'}
        px={{ base: '7px', md: '20px', lg: '50px' }}
        alignItems={'center'}
        py="15px"
      >
        <Text fontSize={'20px'} fontWeight={'bolder'}>
          Tống số user - {user.length}
        </Text>
      </Flex>
      <Box
        w="98%"
        h={{ base: '100vh', md: '100vh', lg: '450px' }}
        direction={'column'}
        overflowY={'scroll'}
      >
        {user?.map((el, ind) => (
          ...
          >
            ...
            >
              ...
              <Box display={{ base: 'block', md: 'none' }}>
                <ShowDetail child={el} fun={fun}>
                  Thông tin chi tiết
                </ShowDetail>
              </Box>
              <Text display={{ base: 'none', md: 'block' }}>
                {el.email.substring(0, 20)}
              </Text>
              <Text display={{ base: 'none', md: 'block' }}>{el.password}</Text>
            </Flex>
            <Flex
              gap={{ base: '30px', md: '30px', lg: '50px' }}
